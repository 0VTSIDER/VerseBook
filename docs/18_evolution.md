...
When developers publish code to the Verse metaverse, they enter into a social contract with all future users of that code. This contract is more than just a convention—it is enforced by the language itself. Consider what happens when you publish a simple value from a module:
...
This seemingly straightforward declaration carries profound implications. By marking `Thing` as public, you are making a commitment that extends indefinitely into the future. An access specifier is only meaningful on a definition that something outside can reach, which is why the value sits at module scope rather than inside a function. Users can depend on `Thing` always existing and always being an integer. While you retain the freedom to change its actual value, the existence and type of `Thing` become permanent fixtures in the metaverse's landscape.
...
A class's finality, uniqueness, abstractness, and concreteness can each change in one direction only, and the permitted direction differs from one to the next:
...
- **Can potentially change abstract class with no class inheritance to interface.** This transformation maintains the same contract, but the compiler does not yet accept it.
...
Structs are stricter still: as noted above, they admit no new fields at all.
...
- **Cannot convert between functions and parametric types.** A function cannot become a type parameter or vice versa, since these are fundamentally different constructs.
...
The asymmetry in these rules reflects *variance* in type theory. Parameters are contravariant, so accepting more general types (widening) is safe. Returns are covariant, so returning more specific types (narrowing) is safe. Effects are covariant too, so having fewer effects is safe.
...
Increasing accessibility is allowed. You can increase the accessibility of definitions: making a private definition public, or protected to public, expands access without breaking existing code. You can equally make constructors more accessible, since allowing more code to construct instances is a safe capability expansion.

Reducing accessibility is forbidden. You cannot reduce the accessibility of definitions, because making a public definition protected, internal, or private breaks all external code using it, and making a protected definition private breaks subclass access. Nor can you make constructors less accessible: a class constructor that was public cannot become private or protected.
...
Non-persistable changes are unconstrained. You can freely add or remove non-persistable types, because types without `<persistable>` do not affect saved data and can be added or removed as needed.

#### Persistent Variables

Verse supports persistent variables that maintain values across sessions, but only in one shape: a persistent variable is a module-scoped `var` whose type is a `weak_map` keyed by a session or a player. A module-scoped `var` of any other type is rejected outright — the compiler asks for a "`session` or `player` key type" — and even a `weak_map` may only be read or written one key at a time. The example below uses `persistent_key`, the key type available outside a Fortnite island.
...
Within that shape, the same add-but-never-remove discipline applies:
...
A parametric type and an ordinary one are also not interconvertible. You cannot convert between parametric and non-parametric forms: a parametric type cannot lose its type parameters, and a regular type cannot gain them.
...
Seven principles are worth remembering. Additions are usually safe: new optional fields, new overloads, new enumerators in open enums. Removals are usually forbidden, because removing public definitions breaks dependent code. Narrowing is often safe, as with more specific return types and fewer effects. Widening is selectively safe, as with more general parameter types (contravariance). Final and non-instance members are the more flexible of the two kinds, since they can evolve types and effects, while non-final instance members are rigid and must maintain exact signatures. Persistable types are strictest of all, because saved data imposes permanent constraints.
